		...
		for $j (0..3)
		{
			...
			for $k (0..99)
			{
				...
				for $l (0..2)
				{
					...
					#/data/users_area/gti10kg/badDiamond/balancedRoot/phylonet/d1/100/0
					$inputFile = "$rooting[$w]". "/phylonet/" . "$diamond[$i]" . "/" . "$ntrees[$j]" . "/" . "$k" . "/" . "$k-$x-$error[$y]" . ".mpl." . "$l" . ".out";			
					...
					if ($l == 1)
					{
						...
						if (length($hybridEdge) < 1)
						{
							$hybridEdge = "#H1";
						}
						open OUT1,'>',"$rooting[$w]/phylonet/$diamond[$i]/$ntrees[$j]/$k/$i.$j.$k-$x-$error[$y].netStats.jl";
...
#						#print OUT1 "using CSV\n";
						...
						if ($edgesCounted == 0)
						{
							open OUT2,'>',"$rooting[$w]/phylonet/$diamond[$i]/$ntrees[$j]/$k/netStats.$k-$x-$error[$y].sh";
							...
							close OUT2;
							system "sbatch $rooting[$w]/phylonet/$diamond[$i]/$ntrees[$j]/$k/netStats.$k-$x-$error[$y].sh";
						}
						elsif ($edgesCounted == 1)
						{
							open FH2,'<',"$rooting[$w]/phylonet/$diamond[$i]/$ntrees[$j]/$k/$i.$j.$k-$x-$error[$y].netStats.txt";
							while(<FH2>)
							{
								$line = $_;
								chomp $line;
