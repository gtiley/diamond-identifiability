		...
		for $j (0..3)
		{
			...
			for $k (0..99)
			{
				...
				for $l (0..2)
				{
					...
					$inputFile = "$rooting[$w]". "/snaq/" . "$diamond[$i]" . "/" . "$ntrees[$j]" . "/" . "$k" . "/" . "$k" . "." . "net" . "$l" . ".true.snaq.out";			
					...
					while (<FH1>)
					{
						if (/(.+;)\s+\-Ploglik\s+\=\s+(\S+)/)
						{
							...
							if ($l == 1)
							{
								...
								open OUT1,'>',"$rooting[$w]/snaq/$diamond[$i]/$ntrees[$j]/$k/$i.$j.$k.netStats.jl";
...
#								#print OUT1 "using CSV\n";
								...
								if ($edgesCounted == 0)
								{
									open OUT2,'>',"$rooting[$w]/snaq/$diamond[$i]/$ntrees[$j]/$k/netStats.sh";
									...
									close OUT2;
#									system "sbatch $rooting[$w]/snaq/$diamond[$i]/$ntrees[$j]/$k/netStats.sh";
								}
								elsif ($edgesCounted == 1)
								{
									open FH2,'<',"$rooting[$w]/snaq/$diamond[$i]/$ntrees[$j]/$k/$i.$j.$k.netStats.txt";
									$tableCorrect = 0;
									while(<FH2>)
									{
										$line = $_;
