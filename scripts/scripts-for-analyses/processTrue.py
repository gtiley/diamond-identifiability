...
if edges_counted == 1:
    filename = f"simulationResults.true.txt"
    ...
    for j in range(3): # phi
        for k in range(3): # number of trees
            ...
            for l in range(20): # number of replicates
                            ...
                            if match:
                                ...
                                if n==1:
                                    ...
                                    if edges_counted == 0:
                                        ...
                                        with open(f"snaq/{diamond[i]}/{phi[i]}_{ntrees[j]}_{length}/{l}/{i}.{j}.{k}.{l}.netStats.txt", 'r') as fh2:
                                            table_correct = False
                                            gamma = None
                                            for line in fh2:
                                                line = line.strip()
                ...
                if edges_counted == 1:
                    out.write(f"{diamond[i]}\t{phi[j]}\t{ntrees[k]}\t{length}\t{l}\t{rank[0]}\t{rank[1]}\t{rank[2]}\t{minh}\t{match}\t{bsp}\t{gamma}\n")
